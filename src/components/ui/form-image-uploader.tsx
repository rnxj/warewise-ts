import { Upload, X } from 'lucide-react';
import { useRef, useState } from 'react';

interface FormImageUploaderProps {
  name: string;
  defaultValue?: string;
  disabled?: boolean;
  className?: string;
  size?: 'sm' | 'lg';
  placeholder?: string;
}

export function FormImageUploader({
  name,
  defaultValue = '',
  disabled = false,
  className = '',
  size = 'lg',
  placeholder,
}: FormImageUploaderProps) {
  const fileInputRef = useRef<HTMLInputElement>(null);
  const hiddenInputRef = useRef<HTMLInputElement>(null);
  const [preview, setPreview] = useState<string>(defaultValue);
  const [error, setError] = useState<string | null>(null);

  const sizeClasses = {
    sm: 'h-16 w-16',
    lg: 'h-20 w-20',
  };

  const iconSizes = {
    sm: 'h-6 w-6',
    lg: 'h-8 w-8',
  };

  const borderClasses = {
    sm: 'border',
    lg: 'border-2 border-muted-foreground/25 border-dashed',
  };

  const handleUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0];
    if (!file) {
      return;
    }

    setError(null);

    if (file.size > 100 * 1024) {
      const errorMsg =
        'Image must be under 100KB. Please choose a smaller file.';
      setError(errorMsg);
      return;
    }

    if (!file.type.startsWith('image/')) {
      const errorMsg = 'Please select a valid image file (PNG, JPG, etc.).';
      setError(errorMsg);
      return;
    }

    const reader = new FileReader();
    reader.onload = (e) => {
      const result = e.target?.result as string;
      setPreview(result);
    };
    reader.readAsDataURL(file);
  };

  const handleRemove = () => {
    setPreview('');
    setError(null);
    if (fileInputRef.current) {
      fileInputRef.current.value = '';
    }
  };

  const handleClick = () => {
    if (!disabled && fileInputRef.current) {
      fileInputRef.current.click();
    }
  };

  return (
    <div className={`flex flex-col items-center gap-2 ${className}`}>
      {/* Hidden input for form submission */}
      <input name={name} ref={hiddenInputRef} type="hidden" value={preview} />

      {/* File input for triggering upload */}
      <input
        accept="image/*"
        className="hidden"
        disabled={disabled}
        onChange={handleUpload}
        ref={fileInputRef}
        type="file"
      />

      <div className="relative">
        {preview ? (
          <>
            <button
              className={`flex ${sizeClasses[size]} items-center justify-center overflow-hidden rounded-lg ${borderClasses[size]} cursor-pointer`}
              onClick={handleClick}
              type="button"
            >
              <img
                alt="Preview"
                className="h-full w-full object-cover"
                height={size === 'sm' ? 64 : 80}
                src={preview}
                width={size === 'sm' ? 64 : 80}
              />
            </button>
            {!disabled && (
              <button
                className="-top-2 -right-2 absolute z-10 flex h-6 w-6 cursor-pointer items-center justify-center rounded-full bg-destructive/80 text-white shadow-sm hover:bg-destructive"
                onClick={handleRemove}
                type="button"
              >
                <X className="h-3 w-3" />
              </button>
            )}
          </>
        ) : (
          <button
            className={`flex ${sizeClasses[size]} items-center justify-center rounded-lg ${borderClasses[size]} ${size === 'lg' ? 'bg-muted/50' : ''} ${disabled ? 'opacity-50' : 'cursor-pointer hover:bg-muted/70'} transition-colors`}
            onClick={handleClick}
            type="button"
          >
            <Upload className={`${iconSizes[size]} text-muted-foreground`} />
          </button>
        )}
      </div>

      {!(preview || error) && placeholder && (
        <p className="text-center text-muted-foreground text-sm">
          {placeholder}
        </p>
      )}

      {error && <p className="text-center text-destructive text-sm">{error}</p>}
    </div>
  );
}
